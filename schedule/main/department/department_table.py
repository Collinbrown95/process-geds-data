...
def create_department_table(df, org_chart):
    '''
    Creates the department table in the database.
    '''
    # Create database connection
    engine = create_engine(assemble_sqlalchemy_url(SQLAlchemyConfig))
    dept_df = df[["dept_id", "department_name"]].drop_duplicates()
    dept_df["org_chart"] = dept_df["department_name"].apply(lambda x: get_department_org_chart(x, org_chart))
...
def get_department_org_chart(department, org_chart):
    ...
    dept = [dept for dept in org_chart["_children"]
